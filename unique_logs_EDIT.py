...
import codecs
dir_list =[]
for filename in os.listdir("C:/python/configs/"):	
...
while files_counter > 0:
	...
	if find_file_name != []:
		for line in open('C:/python/configs/{0}'.format(find_file_name[0]), encoding='utf-8' ).readlines():
	...
	files_counter -= 1
#print ('VISHLI')
new_wb = openpyxl.load_workbook(filename = 'C:/python/outputdir/example.xlsx')
...
while length_clean_file_name_list > 0:
	new_list = []
	for line in open('C:/python/configs/{0}'.format(clean_file_name_list[counter_for_files]), encoding='utf-8' ).readlines():
	...
	#print (event_list)
	#print (row_counter)
new_wb.save('C:/python/outputdir/example.xlsx')
