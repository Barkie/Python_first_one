...
def rulebases(some_str):
  """tryint to find rulebases info in CPinfo"""
  rulebases_final = ''
  rulebases = re.findall(b"----\n.*rulebases_5_0.*\n[-]*\n([\S\s]*?)----------", some_str)
  print(rulebases)
...
for file in listFF:
  ...
  ws = wb.add_sheet('{0}'.format(file), cell_overwrite_ok=True)
  # Setting width
  ...
  row = 1
  # saving workbook
  wb.save('C:/python/outputdir/cpinfo_rules123.xls')
